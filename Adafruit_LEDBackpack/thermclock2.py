...
import time, datetime, scroll, os, sqlite3, json
...
def getwug(dtnow=datetime.datetime.now()):
    # get conditions last reported (cron job updates this)
    returnlist = []
		
    with open('wug.txt') as data_file:
        data = json.load(data_file)

        returnlist.append("out =   " +  int(data["current_observation"]["temp_f"]) + "F")
        returnlist.append("FEELS   " + int(data["current_observation"]["temp_f"]) + "F")
